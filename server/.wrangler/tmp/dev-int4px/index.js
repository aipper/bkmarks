...
// .wrangler/tmp/bundle-Q4Ocgb/checked-fetch.js
...
// .wrangler/tmp/bundle-Q4Ocgb/strip-cf-connecting-ip-header.js
...
async function getSession(KV, sid) {
...
// src/bookmarks.ts
var UTM_PARAMS = ["utm_source", "utm_medium", "utm_campaign", "utm_term", "utm_content", "utm_id", "utm_name", "utm_creative_format", "utm_marketing_tactic", "gclid", "fbclid"];
function normalizeUrl(input) {
  ...
  let s = u.toString();
  if (s.endsWith("/") && u.pathname === "/" && !search)
    s = s.slice(0, -1);
  return s;
}
__name(normalizeUrl, "normalizeUrl");
function ruleTagsFor(url, title) {
  let host = "";
  try {
  ...
  if (host.includes("gitlab.com"))
    tags.add("\u5F00\u53D1");
  if (host.includes("npmjs.com") || host.includes("deno.land"))
    tags.add("\u5305\u7BA1\u7406");
  if (host.includes("stack") || host.includes("stackoverflow.com"))
    tags.add("\u95EE\u7B54");
  if (host.includes("youtube.com") || host.includes("bilibili.com"))
    tags.add("\u89C6\u9891");
  if (host.includes("docs") || host.startsWith("doc") || host.includes("developer"))
    tags.add("\u6587\u6863");
  if (host.includes("medium.com"))
    tags.add("\u9605\u8BFB");
  if (host.includes("google.com") || host.endsWith(".google.com"))
    tags.add("\u641C\u7D22");
  ...
  if (title) {
    ...
    if (t.includes("api") || t.includes("sdk"))
      tags.add("\u63A5\u53E3");
    if (t.includes("guide") || t.includes("\u6307\u5357") || t.includes("\u624B\u518C"))
      tags.add("\u6307\u5357");
  }
  return Array.from(tags);
}
__name(ruleTagsFor, "ruleTagsFor");
...
async function loadIndex(bucket, userId) {
...
async function saveIndex(bucket, userId, idx) {
  const key = `user/${userId}/bookmarks_index.json`;
  idx.updatedAt = Date.now();
  await writeJson(bucket, key, idx);
}
__name(saveIndex, "saveIndex");
async function upsertBookmark(bucket, userId, url, title, tags) {
  const idx = await loadIndex(bucket, userId);
  const key = normalizeUrl(url);
  const now = Date.now();
  const existing = idx.items[key];
  if (existing) {
    const mergedTags = Array.from(/* @__PURE__ */ new Set([...existing.tags || [], ...tags || [], ...ruleTagsFor(key, title || existing.title)]));
    idx.items[key] = { ...existing, title: title || existing.title, tags: mergedTags, updatedAt: now };
  } else {
    const rt = ruleTagsFor(key, title);
    idx.items[key] = { url: key, title, tags: Array.from(/* @__PURE__ */ new Set([...tags || [], ...rt])), createdAt: now, updatedAt: now };
  }
  await saveIndex(bucket, userId, idx);
}
__name(upsertBookmark, "upsertBookmark");
async function removeBookmark(bucket, userId, url) {
  const idx = await loadIndex(bucket, userId);
  const key = normalizeUrl(url);
  if (idx.items[key]) {
...
async function listBookmarks(bucket, userId) {
  const idx = await loadIndex(bucket, userId);
...
// src/status.ts
async function readJson2(bucket, key) {
  const obj = await bucket.get(key);
  if (!obj)
    return null;
  try {
...
async function writeJson2(bucket, key, data) {
  await bucket.put(key, JSON.stringify(data));
}
__name(writeJson2, "writeJson");
async function loadStatus(bucket, userId) {
  const key = `user/${userId}/bookmarks_status.json`;
  return await readJson2(bucket, key) || {};
}
__name(loadStatus, "loadStatus");
async function saveStatus(bucket, userId, map) {
  const key = `user/${userId}/bookmarks_status.json`;
  await writeJson2(bucket, key, map);
}
__name(saveStatus, "saveStatus");
...
// src/ui.ts
function html() {
  return `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Bkmarks</title><link rel="stylesheet" href="/static/app.css"></head><body><div id="app"><header class="topbar"><div class="brand">Bkmarks</div><input id="search" class="search" placeholder="\u641C\u7D22\u4E66\u7B7E"><div id="user" class="user"></div></header><main><div id="tags" class="tags"></div><div id="content"><div id="login" class="panel hidden"><h2>\u767B\u5F55</h2><input id="u" placeholder="\u7528\u6237\u540D"><input id="p" type="password" placeholder="\u5BC6\u7801"><button id="loginBtn">\u767B\u5F55</button></div><div id="list" class="grid"></div></div></main></div><script src="/static/app.js"><\/script></body></html>`;
}
__name(html, "html");
function css() {
...
async function isRegistrationOpen(KV) {
  const flag = await KV.get("system:adminCreated");
  return !flag;
}
__name(isRegistrationOpen, "isRegistrationOpen");
async function closeRegistration(KV) {
  await KV.put("system:adminCreated", "true");
}
__name(closeRegistration, "closeRegistration");
async function openRegistration(KV) {
  await KV.delete("system:adminCreated");
}
__name(openRegistration, "openRegistration");
app.get("/", (c) => {
...
app.post("/api/auth/register", async (c) => {
  ...
  if (!open)
    return c.json({ ok: false, error: "registration_closed" }, 403);
  const exists = await getUser(c.env.KV, username);
  if (exists)
    return c.json({ ok: false, error: "conflict" }, 409);
...
async function getAuthUserFromRequest(c) {
  const auth = c.req.header("Authorization");
  if (auth && auth.startsWith("Basic ")) {
    const raw2 = atob(auth.slice(6));
    const i = raw2.indexOf(":");
    if (i > 0) {
      const username = raw2.slice(0, i);
      const password = raw2.slice(i + 1);
      const user = await getUser(c.env.KV, username);
      if (!user)
        return null;
      const ok = await verifyPassword(password, user.passwordHash);
...
__name(getAuthUserFromRequest, "getAuthUserFromRequest");
...
app.post("/api/bookmarks/sync", async (c) => {
  try {
    ...
    if (t === "created") {
      ...
      if (url)
        await upsertBookmark(c.env.R2, user.id, url, title);
    } else if (t === "changed") {
...
app.post("/api/links/check", async (c) => {
  ...
  const stale = items.filter((it) => {
    ...
    return !s || now - s.lastCheckedAt > 24 * 3600 * 1e3;
  }).slice(0, limit);
...
// .wrangler/tmp/bundle-Q4Ocgb/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-Q4Ocgb/middleware-loader.entry.ts
